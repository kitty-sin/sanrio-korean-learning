...
- **2026-09-18 16:45 PT**：
  - **👑 全新推出《KITTY 韓語兩大敬語發音積木樂園 (`sanrio_korean_honorifics.html`)》Sanrio 萌趣多頁面版**：
    - **背景**：將最高敬語（`하십시오체 / -습니다`）與日常親切敬語（`해요체 / -아요/어요/요`）轉化為具備 Sanrio 角色趣味、5 大分頁切換導航、3 段速語音點讀與隨堂星級闖關測驗的專屬互動樂園。
    - **5 大核心分頁**：
      1. **Page 1: 🌟 樂園總覽與黃金速查總表**（布丁狗 습니다 派 ✕ Hello Kitty 해요 派全景速查對照表、三秒記憶口訣、一鍵點擊聽語音）。
      2. **Page 2: 👑 職場精英「습니다 莊重派」**（動詞/形容詞收音規則、疑問句 `-습니까?`、名詞三大金律 `-입니다 / -입니까? / -이/가 아닙니다`、終聲 ㅂ 鼻音化音變 `[슴니다]` 透視鏡）。
      3. **Page 3: 🌸 生活甜心「해요 親切派」**（母音四大家族拼盤：陽性母音接 `-아요`、陰性及複合母音（含 `ㅐ, ㅖ, ㅢ`）接 `-어요`、中性母音 `ㅡ, ㅣ` 脫落與縮合、`하다` 變 `해요`；名詞肯定 `이에요/예요`、否定 `이/가 아니에요`；魔法 `요` 瞬間升級卡）。
      4. **Page 4: 🥊 實戰 PK 對決台**（自我介紹、用餐問候、否定句、職場 vs 咖啡廳模式即時切換器）。
      5. **Page 5: 🎮 隨堂星級小測驗**（5 題互動闖關測驗 + 答對撒花 Confetti + 即時解析 + 星級成績單）。
    - **全站互聯與離線支援**：
...
- **2026-09-18 23:08 PT**：
  - **🗑️ 移除首頁「⭐ 隨堂星級挑戰」分頁**：
    - 依使用者指示，將首頁中的 `[⭐ 隨堂星級挑戰]` 從 `masterTabs` 及頁面渲染中完整移除。
    - 首頁 Master Tabs 簡化為 **5 大核心大分頁**（`🔤 40音發音積木`、`🧩 3層拼音積木屋`、`🏃 60核心生活動詞`、`🪄 6大音變透視鏡`、`☁️ 雲端自學單字庫`）。
    - `sw.js` 升級至 `v1.0.29`，同步鏡像至 `www/`，推播至 GitHub Pages。
- **2026-09-18 23:19 PT**：
  - **🔄 首頁頂部導航站按鈕順序優化**：
    - 依使用者指示，將 `[📖 發音教學PDF(20頁)]` 與 `[📑 語序敬語時態PDF]` 調整至 `[🔍 查看海報]` 與 `[📚 韓語核心大詞庫]` 之間。
    - `sw.js` 升級至 `v1.0.31`，同步鏡像至 `www/`，推播至 GitHub Pages。
- **2026-09-19 01:38 PT**：
  - **🚂 韓語造句發音積木列車全方位深度升級 (4車卡 ✕ 2排自適應排版 ✕ 獨立現在進行式 ✕ 繁簡雙向翻譯)**：
    - **獨立【現在進行式】專屬框框**：
      - 將【現在進行式】自一般現在式中分離為獨立專屬語法框框，提供最高敬語 `-고 있습니다`、日常敬語 `-고 있어요`、平語 `-고 있어` 完整變化矩陣，每句附獨立 🔊 3 段速語音點讀與中文白話精析。
    - **全面修復羅馬拼音 (Romanization)**：
      - 徹底修復時態助動詞（如 `마실 겁니다`、`읽고 있습니다`）夾雜未轉譯韓文的問題，重構 `HangulEngine.romanize`，精準標記韓語音節連音化與 Revised Romanization。
    - **4 車卡架構升級 (S + P + O + V)**：
      - 擴充為 **4 大車卡**：🚂 車頭 No.1 ＋ 🚃 車卡 1 : 主語 (S) ＋ 🚃 車卡 2 : 地點 (P) ＋ 🚃 車卡 3 : 受語 (O) ＋ 🚃 車卡 4 : 動詞 (V)。
      - 新增地點選單（🪐 太空館、🏠 家裡、🛋️ 客廳、🏫 學校、☕ 咖啡廳、📚 圖書館、🎬 電影院、🌳 公園、🛏️ 房間、🍽️ 餐廳、🏙️ 首爾、✨ 無地點），支援動作地點 `에서` 與目的地 `에` 自動接續。
    - **2 排自適應寬敞大卡排版 (保持車卡大尺寸)**：
      - 為嚴格遵守車卡舒適手感與大字體規範，排版重構為：
        - **第一排 (3 項目)**：🚂 車頭 No.1 ＋ 🚃 車卡 1 : 主語 (S) ＋ 🚃 車卡 2 : 地點 (P)
        - **第二排 (2 項目)**：🚃 車卡 3 : 受語 (O) ＋ 🚃 車卡 4 : 動詞 (V)
    - **繁簡中文智慧翻譯與複合詞自動拆解**：
      - 輸入繁體/簡體中文（如 `太空馆` / `月亮` / `看`）自動翻譯為對應韓文單字；輸入「看月亮」等複合短語自動智慧拆分至受語 `달` 與動詞 `보다`。
    - **修正預設路線**：
      - 路線 #1 精準命名為 **`🪐 弟弟太空館看月亮`**（`동생은 우주관에서 달을 봅니다`）。
    - **同步與部署**：
      - 同步更新 `korean_sentence_data.js`、`sanrio_korean_sentences.html`、`www/` 目錄與 `sw.js`。
      - 已推播至 GitHub Pages（Commits `958d21a`, `c83e952`, `0bd135d`）。
- **2026-09-19 10:30 PT**：
  - **✨ 韓語造句積木列車：四車卡「單一萬能輸入框」深度升級**：
    - 將 4 大車卡之原本雙輸入框（中文 + 韓文）正式**合併為單一萬能輸入框**。
    - 支援繁簡中文、韓文、英文任意語言輸入（如 `John`, `Mary`, `brother`, `太空館`, `看月亮`）。
    - 實作 IME 選字保護（`compositionstart/compositionend`）與專屬 `↵` (Enter) 按鈕，徹底杜絕輸入法提前送出或誤讀問題。
    - 自適應字體 `>= 16px` 防止手機版（iOS/Android）點擊對焦時縮放畫面。
    - `sw.js` 升級至 `v1.0.35`。
- **2026-09-19 10:38 PT**：
  - **🔍 漢字詞智慧轉譯、收音運算與發音引擎排查與修復 (議員/會社/會議/開)**：
    - **問題根因修復**：修正全域詞庫查詢鍵名（支援 `.c/.k` 與 `.zh/.kr`），串接 6,520 筆漢字詞大辭典與逐字元漢字音節矩陣轉譯 (`translateHanjaWord`)。
    - **消除亂碼豆腐字 `冬`**：在 `decomposeChar` / `composeChar` 加入嚴格 Hangul 字元邊界判定，徹底杜絕非韓文字元 `NaN` 運算。
    - **動詞 `ㄹ` 收音脫落規則**：實作 `열다` ➔ `엽니다`，`살다` ➔ `삽니다`，`만들다` ➔ `만듭니다` 正統敬語變形。
    - **羅馬拼音與 TTS 語音朗讀**：擴充專屬羅馬拼音辭典，韓語語音朗讀 100% 自然流暢。
    - `sw.js` 升級至 `v1.0.36`，同步鏡像至 `www/` 目錄並推播至 GitHub Pages（Commit `23b2ef7`）。
- **➡️ 下一步**：
  1. 依學習者進度擴充造句列車的更多預設人氣路線（如美食旅遊、K-POP追星、校園日常）。
  2. 依使用者自訂的拼音規則更新 `hangul_sanrio_deck_20p.pdf`。

---

## 🕐 最後更新資訊
- **更新時間**：2026-09-19 10:45 PT
